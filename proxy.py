...
class Proxy:
    ...
    #Want to creat a ring of pubishers based on the zip and ownership, and then give to the subscribers?
    def create_hashring(self, zipcode, ownership): 
        if [zipcode, ownership] not in self.hashring_servers:
            self.hashring_servers.append([zipcode, ownership])
            pass
        #if the list is not empty
        if self.hashring_servers:
            self.ring = HashRing(hashring_servers)
    ...
    def schedule(self):
        ...
        while True:
            ...
            if self.xsubsocket in events:
                ...
                self.create_hashring(zipcode, ownership)
